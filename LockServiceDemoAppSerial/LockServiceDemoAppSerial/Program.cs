    ...
    class Program
    {
        ...
        private static void LockClients()
        {
            try
            {
                ...
                List<EnsuredSupportUser> ensuredSupportUsers = GetEnsuredSupportUsers();
                        ...
                        try
                        {
                            ...
                            foreach (string siteUrl in data.SiteURLs)
                            {
                                ...
                                try
                                {
                                    ...
                                    //    {
                                    //        foreach (EnsuredSupportUser supportUser in ensuredSupportUsers)
                                    //        {
                                    //            web.EnsureUser(supportUser.UserKey);
                                    //        }
                                    //    }
                                    //    if (data.Action == "lock")
                                    //    {
        ...
        public static List<EnsuredSupportUser> GetEnsuredSupportUsers()
        {
            List<EnsuredSupportUser> result = new List<EnsuredSupportUser>();
            try
            {
                string connectionString = ConfigurationManager.ConnectionStrings["TAXPORTAL"].ConnectionString;
                using (SqlConnection cn = new SqlConnection(connectionString))
                {
                    ...
                    while (rdr.Read())
                    {
                        ...
                        result.Add(new EnsuredSupportUser { UserEmail=rdr.GetString(0),UserKey=rdr.GetString(1)});
                    }
                    rdr.Close();
                    rdr.Dispose();
                }
